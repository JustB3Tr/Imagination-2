#!/usr/bin/env python3
"""
QLoRA supervised fine-tuning for Imagination v1.4 UI Artisan (Stage A — text-only chat JSONL).

Default dataset: ``datasets/ui_artisan_text_sft_seed.jsonl`` under ``versions/v1.4``.

Same training stack as v1.2.1 ``train_imagination.py`` (TRL SFTTrainer + PEFT LoRA).

Example:

  cd versions/v1.4
  python train_ui_v14.py --model_path /path/to/HF_MODEL_DIR --dataset datasets/ui_artisan_text_sft_seed.jsonl --output_dir adapters/0001
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="QLoRA fine-tune Imagination v1.4 UI Artisan (text JSONL)")
    p.add_argument(
        "--model_path",
        type=str,
    ...
    p.add_argument(
        "--dataset",
        type=str,
        default="datasets/ui_artisan_text_sft_seed.jsonl",
